...
const fs = require('fs')
const WorldpayPaymentResponse = require('../../lib/worldpay/WorldpayPaymentResponse')
...
describe('WorldpayPaymentResponse', () => {
  ...
  test('should be invalid with missing merchantCode', () => {
    ...
    expect(response.validate()).toEqual(expect.arrayContaining(["Merchant code can't be blank"]))
  })
  ...
  test('should be invalid with missing orderCode', () => {
    ...
    expect(response.isValid()).toBe(false)
    expect(response.validate()).toEqual(expect.arrayContaining(["Order code can't be blank"]))
  })
  ...
  test('should be invalid with missing referenceId', () => {
    ...
    expect(response.isValid()).toBe(false)
    expect(response.validate()).toEqual(expect.arrayContaining(["Reference id can't be blank"]))
  })
  ...
  test('should be invalid with missing referenceValue', () => {
    ...
    expect(response.isValid()).toBe(false)
    expect(response.validate()).toEqual(expect.arrayContaining(["Reference value can't be blank"]))
  })
  ...
  test('should be invalid with referenceValue that is not a URL', () => {
    ...
    expect(response.isValid()).toBe(false)
    expect(response.validate()).toEqual(
      expect.arrayContaining(['Reference value is not a valid url'])
    )
  })
