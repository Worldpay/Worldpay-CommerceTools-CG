...
const { WORLDPAY_JOURNAL_TYPES: WJT } = require('../../src/worldpay/constants')
const { parseXml } = require('../../src/worldpay/xmlParser')

jest.mock('../../src/worldpay/xmlParser')
...
describe('WorldpayNotification', () => {
  ...
  beforeEach(() => {
    ...
    extractXmlObjectToPropertiesSpy = jest.spyOn(WorldpayNotification.prototype, 'extractXmlObjectToProperties')
  ...
  describe('determineAmount', () => {
    ...
    it('should determine the correct amount for a AUTHORISED notification', () => {
      const notification = new WorldpayNotification(getTestXml('notificationValid1'))
      const amount = notification.determineAmount(WJT.AUTHORISED, require('./data/xmlObjects/authorised'))
      expect(amount).toEqual({
        centAmount: 7500,
        currencyCode: 'GBP',
        fractionDigits: 2,
    ...
    it('should determine the correct amount for a CAPTURED notification', () => {
      const notification = new WorldpayNotification(getTestXml('notificationValid1'))
      const amount = notification.determineAmount(WJT.CAPTURED, require('./data/xmlObjects/captured'))
      expect(amount).toEqual({
        centAmount: 9520,
        currencyCode: 'USD',
        fractionDigits: 2,
    ...
    it('should determine the correct amount for a REFUNDED notification', () => {
      const notification = new WorldpayNotification(getTestXml('notificationValid1'))
      const amount = notification.determineAmount(WJT.REFUNDED, require('./data/xmlObjects/refunded'))
      expect(amount).toEqual({
        centAmount: 1600,
        currencyCode: 'EUR',
        fractionDigits: 2,
    ...
    it('should determine the correct amount for a CANCELLED notification', () => {
      const notification = new WorldpayNotification(getTestXml('notificationValid1'))
      const amount = notification.determineAmount(WJT.CANCELLED, require('./data/xmlObjects/cancelled'))
      expect(amount).toEqual({
        centAmount: 12800,
        currencyCode: 'EUR',
        fractionDigits: 2,
    ...
    it('should return null when the amountTx property is missing', () => {
      const notification = new WorldpayNotification(getTestXml('notificationValid1'))
      const amount = notification.determineAmount(WJT.AUTHORISED, require('./data/xmlObjects/authorisedEmptyAccountTx'))
  ...
  describe('extractXmlObjectToProperties', () => {
    ...
    it('should throw an error when the notification is missing the `orderStatusEvent` element', () => {
      expect(() => {
        ...
        notification.extractXmlObjectToProperties(invalidObject)
      }).toThrow(new Error('Worldpay response XML missing expected paymentService.notify.orderStatusEvent node'))
    })
    ...
    it('should extract XML object properties to instance properties', () => {
      ...
      expect(notification.orderCode).toBe(VALID_XML_OBJECT.paymentService.notify.orderStatusEvent['@orderCode'])
      expect(notification.worldpayDtdVersion).toBe(VALID_XML_OBJECT.paymentService['@version'])
    })

    it('should not extract extract token properties if not present', () => {
      notification.extractXmlObjectToProperties(VALID_XML_OBJECT)
      expect(notification.token).toBeUndefined()
    })
  })
})
