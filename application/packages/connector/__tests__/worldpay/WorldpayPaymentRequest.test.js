...
const WorldpayAddress = require('../../src/worldpay/WorldpayAddress')
const { WorldpayValidationException } = require('../../src/worldpay/xmlBuilder')
const WorldpayPaymentMethodMask = require('../../src/worldpay/WorldpayPaymentMethodMask')
const WorldpayAuthenticationRiskData = require('../../src/worldpay/WorldpayAuthenticationRiskData')
const WorldpayShopperAccountRiskData = require('../../src/worldpay/WorldpayShopperAccountRiskData')
const WorldpayTransactionRiskData = require('../../src/worldpay/WorldpayTransactionRiskData')
const WorldpayCreateTokenData = require('../../src/worldpay/WorldpayCreateTokenData')
const WorldpayFraudSightData = require('../../src/worldpay/WorldpayFraudSightData')
...
describe('WorldpayPaymentRequest', () => {
  test('should be valid full Worldpay XML with all optional data added', () => {
    const payment = new WorldpayPaymentRequest('TESTMERCHCODE', 'TESTINSTALLATION')
      ...
      .withShopperDetails(new WorldpayShopper('bob@foo.com', '12345').withBrowserAcceptHeader('ACCEPT_HEADER'))
      .withShippingAddress(
        new WorldpayAddress()
      ...
      .withShopperAccountRiskData(
        new WorldpayShopperAccountRiskData()
          ...
          .withShopperAccountPasswordChangeIndicator(WP.WORLDPAY_ACCOUNT_INDICATOR_LESS_THIRTY_TO_SIXTY_DAYS)
          .withShopperAccountShippingAddressUsageIndicator(WP.WORLDPAY_ACCOUNT_INDICATOR_MORE_THAN_SIXTY_DAYS)
      ...
      .withCreateTokenData(
        new WorldpayCreateTokenData()
          .withTokenScope('shopper')
          .withTokenOptIn('ask')
          .withTokenEventReference('Order_123')
          .withTokenReason('A very important reason')
          .withPaymentTokenExpiry(new Date('January 10, 2000 01:23:45')),
      )
      ...
      <paymentService version="1.4" merchantCode="TESTMERCHCODE">
        <submit>
          <order orderCode="TESTORDERCODE" installationId="TESTINSTALLATION" captureDelay="0">
            ...
            <createToken tokenScope="shopper" tokenOptIn="ask">
              <tokenEventReference>Order_123</tokenEventReference>
              <tokenReason>A very important reason</tokenReason>
              <paymentTokenExpiry>
  ...
  test('should be valid full Worldpay XML with only mandatory elements', () => {
    const payment = new WorldpayPaymentRequest('TESTMERCHCODE', 'TESTINSTALLATION')
      ...
      .withPaymentAmount(new WorldpayAmount('GBP', 2500))
      .withShopperDetails(new WorldpayShopper('bob@foo.com', '12345'))
  ...
  test('should be invalid with all missing properties', () => {
    ...
    expect(payment.validate()).toEqual(
      expect.arrayContaining([
        ...
        "InstallationId can't be blank",
        "MerchantCode can't be blank",
        "OrderCode can't be blank",
        "OrderDescription can't be blank",
      ]),
    )
  ...
  test('should be invalid with only shopper risk data', () => {
    const payment = new WorldpayPaymentRequest().withShopperAccountRiskData(
      new WorldpayShopperAccountRiskData().withShopperAccountCreationDate(new Date('January 10, 2000 01:23:45')),
    )
    expect(payment.isValid()).toBe(false)
    const riskData = payment.buildRiskData()
    expect(riskData).toBeDefined()
  ...
  test('should be invalid with a nested invalid email and amount', () => {
    const payment = new WorldpayPaymentRequest('TESTMERCHCODE', 'TESTINSTALLATION')
      .withOrderDetails('TESTORDERCODE', 'TEST_ORDER_DESC')
      .withPaymentMethodMask(new WorldpayPaymentMethodMask().includeOnline())
      .withPaymentAmount(new WorldpayAmount('ZZZ', 123))
      .withShopperDetails(new WorldpayShopper('invalid_email'))

    expect(payment.validate()).toEqual(
      expect.arrayContaining([
        'EmailAddress [invalid_email] is not a valid email',
        'CurrencyCode [ZZZ] is not a valid ISO4217 code',
      ]),
    )
    expect(payment.isValid()).toBe(false)
  })
