...
class WorldpayNotification {
  ...
  extractXmlObjectToProperties(xmlObject) {
    ...
    if (!xmlObject.paymentService.notify.orderStatusEvent) {
      throw new Error('Worldpay response XML missing expected paymentService.notify.orderStatusEvent node')
    }
    ...
    this.token = xmlObject.paymentService.notify.orderStatusEvent?.token
